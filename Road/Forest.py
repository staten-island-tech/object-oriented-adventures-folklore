...
class forestman():
    ...
    def forester():
        while True:
            player = input("C (COUNTINE) /S (STOP)").upper()
            if player == 'C':
                ...
                os.system('cls')
                global direction
                direction = input("A (left), W (Foward), S (Backward), D (right)").upper()
                os.system('cls')
            elif player == 'S':
                input("You choose off. This means you are quiting the game. Are you sure? YES/NO")
                time.sleep(2)
                os.system('cls')
                print("Loading.... Game is turning off  Bye bye!")
                sys.exit()
            else:
                os.system('cls')
                print("Invalid Input: Remember that your input should be either C (COUNTINE) /S (STOP)")
                player = input("C (COUNTINE) /S (STOP)").upper()
